...
public class Carousel : MonoBehaviour
{
    ...
    IEnumerator MoveCarousel(int direction) // direction = -1 (right) / 1 (left)
    {
        ...
        Vector3[] endPositions = new Vector3[items.Length];
        ...
        for (int i = 0; i < items.Length; i++)
        {
            if (items[i].localPosition.x > spacing * (items.Length / 2))
            {
                items[i].localPosition = new Vector3(
                    items[i].localPosition.x - spacing * items.Length,
                    items[i].localPosition.y,
                    items[i].localPosition.z
                );
            }
            else if (items[i].localPosition.x < -spacing * (items.Length / 2))
            {
                items[i].localPosition = new Vector3(
                    items[i].localPosition.x + spacing * items.Length,
                    items[i].localPosition.y,
